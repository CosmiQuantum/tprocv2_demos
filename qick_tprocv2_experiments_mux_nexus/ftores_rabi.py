...
class FtoResAmplitudeRabiExperiment:
    def __init__(self, QubitIndex, number_of_qubits, list_of_all_qubits,  outerFolder, round_num, signal, save_figs, experiment = None, live_plot = None,
        ...
        self.expt_name = 'len_rabi_ftores'
        ...
        if experiment is not None:
            ...
            print(f'Q {self.QubitIndex + 1} Round {self.round_num} FtoRes Rabi configuration: ', self.config)


    def run(self, soccfg, soc):
        amp_rabi = FtoResAmplitudeRabiProgram(soccfg, reps=self.config['reps'], final_delay=self.config['relax_delay'], cfg=self.config)
        ...
        self.plot_results(I, Q, lengths, config=self.config)
        return I, Q, lengths#, q1_fit_cosine, pi_amp, self.config
    ...
    def plot_results(self, I, Q, lengths, config = None, fig_quality = 100):
        #try:
        ...
        #              f"FtoRes Rabi Q{self.QubitIndex + 1}: {pi_amp} (a.u.) _"  + f", {config['reps']}*{config['rounds']} avgs",
        ...
        # else:
        #     fig.text(plot_middle, 0.98,
        #              f"FtoRes Rabi Q{self.QubitIndex + 1}: {pi_amp} (a.u.)_" f", {self.config['sigma'] * 1000} ns sigma" + f", {self.config['reps']}*{self.config['rounds']} avgs",
        #              fontsize=24, ha='center', va='top')

        ax1.plot(lengths, I, label="Gain (a.u.)", linewidth=2)
        ax1.set_ylabel("I Amplitude (a.u.)", fontsize=20)
        ax1.tick_params(axis='both', which='major', labelsize=16)

        ax2.plot(lengths, Q, label="Q", linewidth=2)
...
class FtoResAmplitudeRabiProgram(AveragerProgramV2):
    def _initialize(self, cfg):
        ...
        self.add_pulse(ch=qubit_ch, name="gepi_pulse",
                       style="arb",
                       envelope="ge_ramp",
        ...
        self.add_gauss(ch=qubit_ch, name="ef_ramp", sigma=cfg['sigma_ef'], length=cfg['sigma_ef'] * 4, even_length=False)
        self.add_pulse(ch=qubit_ch, name="efpi_pulse",
                       style="arb",
                       envelope="ef_ramp",
        ...
        #self.add_gauss(ch=qubit_ch, name="ramp", sigma=cfg["sigma_ftores"], length=cfg["sigma_ftores"] * 4, even_length=False)
        self.add_pulse(ch=qubit_ch, name="qubit_pulse",
                       style="const",
                       length=cfg["qubit_length_ftores"],

                       freq=cfg['qubit_freq_ftores'],
                       phase=cfg['qubit_phase'],
                       gain=cfg['qubit_gain_ftores'],
                       )

        self.add_loop("lengloop", cfg["steps"])

    def _body(self, cfg):
        self.pulse(ch=self.cfg["qubit_ch"], name="gepi_pulse", t=0)
        self.delay_auto(t=0.01, tag='waiting after pige')

        self.pulse(ch=self.cfg["qubit_ch"], name="efpi_pulse", t=0)
        self.delay_auto(t=0.01, tag='waiting after pief')

        self.pulse(ch=self.cfg["qubit_ch"], name="qubit_pulse", t=0)
        self.delay_auto(t=0.0, tag='waiting after pi ftores')
