...
stop_time=np.linspace(150,600,10)
stop_time=[round(n, 0) for n in stop_time]
batch_num=0
j = 0
angles=[]
for stop in  stop_time:
    ...
    for QubitIndex in Qs_to_look_at:
        ...
        expt_cfg["T1_ge"]["stop"][QubitIndex]=stop
    ...
    if save_data_h5:
        ...
        if j % save_r == 0:
            ...
            # --------------------------save t1-----------------------
            if run_flags["t1"]:
                outerFolder = os.path.join(outerFolder, str(stop))
