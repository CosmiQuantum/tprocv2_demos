...
from section_012_Parity import Parity_ef
    ...
    for QubitIndex in Qs_to_look_at:
        ...
        if run_flags["Parity"]:
            try:
                par = Parity_ef(QubitIndex, tot_num_of_qubits, studyDocumentationFolder, j, save_figs, experiment = experiment,
                                verbose = verbose, logger = rr_logger)
                iq_list_f, Par_config = par.runParity()
                I = iq_list_f[QubitIndex][0].T[0]
                Q = iq_list_f[QubitIndex][0].T[1]
                # I_e = iq_list_e[QubitIndex][0].T[0]
                # Q_e = iq_list_e[QubitIndex][0].T[1]
        ...
        if save_data_h5:
            ...
            if run_flags["Parity"]:
                ...
                #     time.mktime(datetime.datetime.now().timetuple()))
                par_data[QubitIndex]['I'][j - batch_num * save_r - 1] = I
                par_data[QubitIndex]['Q'][j - batch_num * save_r - 1] = Q
