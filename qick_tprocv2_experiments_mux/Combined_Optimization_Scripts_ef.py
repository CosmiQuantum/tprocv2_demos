...
for QubitIndex in Qs:
    ...
    efrabi = EF_AmplitudeRabiExperiment(QubitIndex, number_of_qubits, outerfolder_plots, j, signal,
                                   False, experiment=experiment, live_plot=live_plot,
...
#                     fids.append(fid)
#                 elif fid_states == 'ef':
#                     ss = SingleShot_ef(QubitIndex, number_of_qubits, outerFolder,  j, save_figs, experiment, unmasking_resgain = unmask)  # updated way
...
#     list_of_all_qubits = [0, 1, 2, 3, 4, 5]
    ...
    nowt=time.time()
    optimal_lengths =  [4.1, 4.0, 3.5, 6.0, 3.5, 4.1] #optional # DAC 2 optimization
    ...
    gain_range = [0.1, 0.6]  # Gain range in a.u.
    freq_steps = 15
    gain_steps = 14
    n_loops = 1
    ...
    experiment = copy.deepcopy(tuned_experiment)
    sweep = GainFrequencySweep(QubitIndex, number_of_qubits, n_loops, list_of_all_qubits, experiment, optimal_lengths=optimal_lengths, output_folder=output_folder, unmasking_resgain = unmask)
    results = sweep.run_sweep(freq_range, gain_range, freq_steps, gain_steps, fid_states)
