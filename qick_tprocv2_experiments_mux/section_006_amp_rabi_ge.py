...
class AmplitudeRabiExperiment:
    ...
    def run(self, thresholding=False, use_iminuit_instead = True):
        ...
        if self.QZE:
            ...
            print("using self.config['sigma']: ",self.config['sigma'])
            ...
            if self.live_plot:
                ...
                if thresholding:
                    iq_list = amp_rabi.acquire(self.experiment.soc, soft_avgs=self.config["rounds"], threshold=self.experiment.readout_cfg["threshold"],
                                               angle=self.experiment.readout_cfg["ro_phase"], progress=self.qick_verbose)
                else:
    ...
    def run_active_reset(self, scaling=True, control_test=False):
        ...
        if scaling:
            ...
            ss_I_e = iq_list_e[self.QubitIndex][0].T[0]
...
####################### Arianna is currently testing the class below ###################################################################
