...
class SingleShot_ef:
    ...
    def run(self, soccfg, soc):
        ...
        ie_new, qe_new, if_new, qf_new, theta_ef, threshold_ef = self.plot_results(iq_list_g, iq_list_e,iq_list_f,self.QubitIndex)
        # return fid, angle, iq_list_g, iq_list_e, iq_list_f
        return iq_list_g, iq_list_e, iq_list_f, ie_new, qe_new, if_new, qf_new, theta_ef, threshold_ef, self.config
    ...
    def plot_results(self, iq_list_g, iq_list_e, iq_list_f, QubitIndex, fig_quality=100):
        ...
        ie_new, qe_new, if_new, qf_new, theta_ef, threshold_ef = self.hist_ssf(data=[I_e, Q_e, I_f, Q_f], cfg=self.config, plot=self.save_figs, fig_quality=fig_quality)
...
class GainFrequencySweep:
    def __init__(self, qubit_index, experiment, num_qubits, n_loops, optimal_lengths=None,  output_folder="/default/path/"):
    ...
    def run_sweep(self, freq_range, gain_range, freq_steps, gain_steps, fid_states):
        ...
        for freq_step in range(freq_steps):
            ...
            for gain_step in range(gain_steps):
                ...
                round_num = 0
                save_figs = False
                # single_shot = SingleShot_ef(self.qubit_index, self.output_folder, round_num, save_figs,
                #                             fresh_experiment)  # SingleShot(self.qubit_index, self.output_folder, fresh_experiment, round_num=0, save_figs = False)
                ...
                for k in range(self.n_loops):  # loops for each read out length
                    ...
                    save_figs = False
                    if fid_states=='ge':
                        single_shot = SingleShot(self.qubit_index, self.num_qubits, self.output_folder, round_num,
                                                 save_figs, fresh_experiment)
                        fidelity = single_shot.fidelity_test(fresh_experiment.soccfg, fresh_experiment.soc)
                        fids.append(fidelity)
                    elif fid_states=='ef':
                        single_shot = SingleShot_ef(self.qubit_index, self.num_qubits, self.output_folder,
                                                    round_num,
                                                    save_figs, fresh_experiment)
                        fidelity = single_shot.fidelity_test(fresh_experiment.soccfg, fresh_experiment.soc)
                        fids.append(fidelity)
