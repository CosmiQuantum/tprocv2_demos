...
class SingleShot:
    ...
    def run(self, return_centers = False, active_reset = False):
        if active_reset:
            ...
            ssp_e = SingleShotProgram_e_active_reset(self.experiment.soccfg, reps=1, final_delay=20,cfg=self.config)
    ...
    def hist_ssf_active_reset(
        ...
        if plot == True:
            ...
            axs[0].set_title('Pre-rotated')
            axs[0].axis('equal')

        # The incoming IQ data have already been rotated.
        # Do not apply an additional software rotation here.
        theta = 0.0

        ig_new = np.asarray(ig)
        qg_new = np.asarray(qg)
        ie_new = np.asarray(ie)
        qe_new = np.asarray(qe)
        ...
        if plot == True:
            ...
            axs[1].set_title('No Additional Rotation')
            axs[1].axis('equal')
            ...
            ng, binsg = np.histogram(
                ig_new,
                bins=numbins,
                range=xlims
            )

            ne, binse = np.histogram(
                ie_new,
                bins=numbins,
                range=xlims
            )
        ...
        if plot == True:
            ...
            axs[2].set_title(
                f"Q{QubitIndex + 1} Fidelity = {fid * 100:.2f}%"
            )
        ...
        if plot == True:
            ...
            outerFolder_expt = os.path.join(
                self.outerFolder,
                "ss_ge_plots_active_reset"
            )
            self.create_folder_if_not_exists(outerFolder_expt)

            outerFolder_expt = os.path.join(
                outerFolder_expt,
                "Q" + str(self.QubitIndex + 1)
            )
            self.create_folder_if_not_exists(outerFolder_expt)
...
class GainFrequencySweep:
    def __init__(self,qubit_index, number_of_qubits, list_of_all_qubits, experiment, optimal_lengths=None, output_folder="/default/path/", unmasking_resgain = False, save_figs = False):
...
class SingleShotProgram_g_active_reset(AveragerProgramV2):
    ...
    def _active_reset_block(self, cfg, label_addition=""):
        ...
        #
        #     # Ground-like shots skip the correction pi pulse.
        #     self.read_and_jump(ro_ch=ro_ch_this, component="I",
        #                        threshold=int(np.round(cfg["threshold"])), test="<",
        #                        label=f"no_pi_{label_addition}_{i}")
        #
        #     # Excited-like shots receive the correction pi pulse.
        ...
        # if you don't want to apply active reset in this program, use pass.
        pass
...
class SingleShotProgram_e_active_reset(AveragerProgramV2):
    ...
    def _active_reset_block(self, cfg, label_addition=""):
        ...
        for i in range(n_resets):
            ...
            self.pulse(ch=self.cfg["qubit_ch"], name="qubit_pulse", t=0)
            self.delay_auto(t=10) # time to empty out the resonator of residual photons. Should ideally be 4/kappa.
