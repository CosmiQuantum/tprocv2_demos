...
class LengthRabiExperiment:
    def __init__(self, QubitIndex, number_of_qubits, outerFolder, round_num, signal, save_figs, experiment = None,
        ...
        if self.QZE:
            ...
            self.expt_name = "length_rabi_Qtemps"
            self.Qubit = 'Q' + str(self.QubitIndex)
            self.exp_cfg = expt_cfg[self.expt_name]
            self.round_num = round_num
            self.live_plot = live_plot
    ...
    def run_rabiforQtemps(self, thresholding=False, constant_zeno_pulse=False):
        amp_rabi1 = LengthRabiProgram1(
            self.experiment.soccfg,
            reps=self.config['reps'],
            final_delay=self.config['relax_delay'],
            cfg=self.config
        )
        iq_list1 = amp_rabi1.acquire(
            self.experiment.soc,
        ...
        lengths1 = amp_rabi1.get_pulse_param('qubit_pulse_ef', "length", as_array=True)
        q1_fit_cosine_1, pi_len_1 = self.plot_results( I1, Q1, lengths1, config = self.config)

        amp_rabi2 = LengthRabiProgram2(
            self.experiment.soccfg,
            reps=self.config['reps'],
            final_delay=self.config['relax_delay'],
            cfg=self.config
        )
        iq_list2 = amp_rabi2.acquire(
            self.experiment.soc,
            soft_avgs=self.config["rounds"],
            progress=self.qick_verbose
        )
        ...
        # get the lens that were used so you can use to plot on the x axis
        lengths2 = amp_rabi2.get_pulse_param('qubit_pulse_ef', "length", as_array=True)
        q1_fit_cosine_2, pi_len_2 = self.plot_results(I2, Q2, lengths2, config=self.config)


        return I1, Q1, lengths1, q1_fit_cosine_1, pi_len_1, I2, Q2, lengths2, q1_fit_cosine_2, pi_len_2, self.config
    ...
    def plot_results(self, I, Q, lens, config = None, fig_quality = 100, showfig=False):
        try:
            ...
            if self.save_figs:
                ...
                print('Plot saved to: ', outerFolder_expt)
            plt.close(fig)
...
class LengthRabiProgram1(AveragerProgramV2):
    ...
    def _body(self, cfg):
        ...
        self.pulse(ch=self.cfg["qubit_ch"], name="qubit_pulse_ef", t=0)  # f-e pulse
...
class LengthRabiProgram2(AveragerProgramV2):
    def _initialize(self, cfg):
        ro_ch = cfg['ro_ch']
        res_ch = cfg['res_ch']
        qubit_ch = cfg['qubit_ch']
    ...
    def _body(self, cfg): #here
        #self.send_readoutconfig(ch=cfg['ro_ch'], name="myro", t=0)
        self.pulse(ch=self.cfg["qubit_ch"], name="pi_ge", t=0)  # play ge pi pulse
        self.delay_auto(t=0.01, tag='waiting after pi')  # Wait til ge pi pulse is done before proceeding

        self.pulse(ch=self.cfg["qubit_ch"], name="qubit_pulse_ef", t=0)  # f-e pulse
        self.delay_auto(t=0.01, tag='waiting')  # wait

        self.pulse(ch=self.cfg["qubit_ch"], name="pi_ge", t=0)  # play ge pi pulse
        self.delay_auto(t=0.01, tag='waiting after 2nd pi')  # Wait til ge pi pulse is done before proceeding
