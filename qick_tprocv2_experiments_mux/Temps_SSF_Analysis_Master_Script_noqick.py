...
for full_path in paths:
    ...
    for QubitIndex in Science_Qubits: # We are only taking science data for qubits 1 and 5
        try:
            ...
            print(f"Failed loading Qspec for qubit {QubitIndex} from {full_path}: {e}")
...
########################################## Qubit Temperatures via SSF method ###########################################
temps_class_obj = TempCalcAndPlots(list_of_all_qubits,figure_quality, figure_quality, tot_num_of_qubits, dataset, save_figs,
              fit_saved, signal, run_name, path)
