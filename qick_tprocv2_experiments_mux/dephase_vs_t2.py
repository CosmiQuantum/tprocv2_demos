...
class DephaseT2Plots:
    ...
    def plot_with_errs2(self, date_times_r, t2r_vals, t2r_fit_err, date_times_e, t2e_vals, t2e_fit_err, show_legends):
        # ---------------------------------plot-----------------------------------------------------                                                                                              
        analysis_folder = f"{self.outerFolder_save_plots}/features_vs_time/"
        self.create_folder_if_not_exists(analysis_folder)
        ...
        for i, ax in enumerate(axes):
            ...
            t2_prop = np.array(sorted_y) / np.array(sorted_y2)
            ...
            pure_dephase_uncert = np.abs(pure_dephase * np.sqrt((t2r_rel_uncer)**2 + (t2e_rel_uncer)**2))
            ...
            ax.errorbar(
                sorted_y, pure_dephase, xerr=sorted_err, yerr=pure_dephase_uncert,
                fmt='o',
                markersize=3,
                label='Dephase',
                color=colors[i],
            ...
            if show_legends:
                ax.legend(edgecolor='black')
            ax.set_xlabel('T2R', fontsize=font - 2)
