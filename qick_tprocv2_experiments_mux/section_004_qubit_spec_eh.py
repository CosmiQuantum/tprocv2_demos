...
class EHQubitSpectroscopy:
    def __init__(self, QubitIndex, number_of_qubits,  outerFolder,  round_num, signal, save_figs, experiment = None,
        ...
        if experiment is not None:
            ...
            print(f'Q {self.QubitIndex + 1} Round {self.round_num} EH Qubit Spec configuration: ', self.config)

    def run(self, return_fwhm=False):
        if self.increase_reps:
            self.config['reps'] = self.increase_reps_to

        ehqspec = EHPulseProbeSpectroscopyProgram(self.experiment.soccfg, reps=self.config['reps'], final_delay=0.5, cfg=self.config)

        # iq_lists= []
        if self.live_plot:
            efI, efQ, effreqs = self.live_plotting(ehqspec, self.experiment.soc)
        else:
            ehiq_list = ehqspec.acquire(self.experiment.soc, soft_avgs=self.exp_cfg["rounds"], progress=self.qick_verbose)
            ehI = ehiq_list[self.QubitIndex][0, :, 0]
            ehQ = ehiq_list[self.QubitIndex][0, :, 1]
            ehfreqs = ehqspec.get_pulse_param('qubit_pulse', "freq", as_array=True)
            #print(effreqs)
    ...
    def plot_results(self, I, Q, freqs, config=None, fig_quality=100):
        ...
        #              f"EH Qubit Spectroscopy Q{self.QubitIndex + 1}, %.2f MHz" % largest_amp_curve_mean +
        ...
        plt.close(fig)
        return None, None, None, #largest_amp_curve_mean, I_fit, Q_fit
...
class EHPulseProbeSpectroscopyProgram(AveragerProgramV2):
    def _initialize(self, cfg):
        ...
        self.declare_gen(ch=cfg['qubit_ampl_ch'], nqz=1, mixer_freq=cfg['qubit_mixer_freq2'])
        self.add_pulse(ch=cfg['qubit_ampl_ch'], name="qubit_pulse", ro_ch=ro_ch[0],
                       style="const",
                       length=cfg['qubit_length_ge'],
                       freq=cfg['qubit_freq_eh'],
                       phase=0,
                       gain=cfg['qubit_gain_eh'],
                       )
