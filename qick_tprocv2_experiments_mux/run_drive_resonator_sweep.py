...
run = "run8"
study = "debugging" #'drive_resonator'
sub_study = 'get_timestep' #'2xamp_S7_sweep_pre100_drive50_post100_delay50'
substudy_txt_notes = 'collecting timestream data'
sensor = [0]
drive_list = [0]
drive_gain = [0.05] #[0.01, 0.05, 0.1, 0.3, 0.45]
sensor_gain = 1.0
...
## get frequency of sensor resonator
ResonatorIndex = sensor[0]
rspec_data = create_data_dict(rspec_keys, save_r, sensor)
timestamp_rspec = time.mktime(datetime.datetime.now().timetuple())
r_spec = ResonanceSpectroscopy(ResonatorIndex, 1, studyDocumentationFolder, 0,
...
fig.suptitle(f"sensor resonator R{sensor}, sensor gain={sensor_gain}")
ax_1 = -1
ax_0 = 0

for d in drive_list:
    drive = [d]
    ResonatorIndex = drive[0]
    ax_1 = ax_1 + 1
    if ax_1 > 3:
        ax_1 = 0
        ax_0 = 1
    ...
    for g in drive_gain:
        rspec_data = create_data_dict(rspec_keys, save_r, drive)
        timestamp_rspec = time.mktime(datetime.datetime.now().timetuple())
        r_spec = ResonanceSpectroscopy(ResonatorIndex, len(drive), studyDocumentationFolder, 0,
            save_figs=save_figs, experiment=experiment,verbose=verbose, logger=rr_logger, dac='fsgen', gain=g)
        freq_sweep, I, Q, gain_sweep, sys_config = r_spec.run()
        ...
        drive_data = create_data_dict(drive_keys, save_r, drive)
        timestamp_drive = time.mktime(datetime.datetime.now().timetuple())
        print(f"drive gain is {g}")
        stream = ResonatorDrive(drive, sensor, len(sensor), studyDocumentationFolder, dataSetFolder, 0,
                               save_figs=save_figs, experiment=experiment,
                               verbose=verbose, logger=rr_logger, drive_gain=g)
        ...
        stream.plot_timestreamv2(I_stack, Q_stack, amp_stack, amp_block_IQ_stack, block_t_stack, fig, ax[ax_0][ax_1])
        ...
        if save_data_h5:
                ...
                drive_data[ResonatorIndex]['Round Num'][0] = 0
