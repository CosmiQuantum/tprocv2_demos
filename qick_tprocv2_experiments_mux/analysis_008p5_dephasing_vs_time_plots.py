...
class DephasingVsTime:
    ...
    def run_T1(self,return_errs=False,return_noise_gain=False,return_freq_offset=False,name='Dephased_ge', savefigs=False, fit=False, batch_nums=False):
        import datetime
        self.name=name
        # ----------Load/get data------------------------
                                ...
                                if fit:
                                    ...
                                    if savefigs:
                                        self.plot_results(I, Q, delay_times, q1_fit_exponential, T1_est,
                                                          ...
                                                          outerFolder=outerFolder_save_plots, expt_name=name,coherence_type='T1')
                                    if T1_est < 0:
                                    ...
                                    if T1_est > 1000:
                                        ...
                                        continue
                                    if T1_err >= 0.3 * T1_est:
                                        print(
                                            f"Skipping T1 = {T1_est:.3f} µs because its error {T1_err:.3f} µs is >= 30% of its value.")
                                        continue
    ...
    def run(self,return_errs=False,return_noise_gain=False,return_freq_offset=False,name='Dephased_ge', savefigs=False, filters=False, batch_nums=False,save_plot_folder=''):
        import datetime
        self.name=name
        # ----------Load/get data------------------------
                            ...
                            try:
                                ...
                                if savefigs:
                                    self.plot_results(I, Q, delay_times,  fitted, t2e_est,
                                                     t2e_err, config=None, fig_quality=100,outerFolder=save_plot_folder, expt_name=name)
    ...
    def plot_with_errs_subtract_T1(
        ...
        # ─────────────────────────────── style registry ───────────────────────────
        DATASETS = {
            "t1": dict(dt=date_times_t1, y=t1_vals, err=t1_fit_err, batch=t1_batch_num,
            ...
            "t2e_w_noise": dict(dt=date_times_t2e_w_noise, y=t2e_vals_w_noise, err=t2e_fit_err_w_noise, batch=t2e_batch_num_w_noise,
                                label="Spin Echo + EF noise"+extra_label, color="blue", marker="x"),
            "t2r_w_noise": dict(dt=date_times_t2r_w_noise, y=t2r_vals_w_noise, err=t2r_fit_err_w_noise, batch=t2r_batch_num_w_noise,
                                label="Ramsey + EF noise"+extra_label, color="gray", marker="d"),
            "dephasing_ef": dict(dt=date_times_dephasing_ef, y=dephasing_vals_ef, err=dephasing_fit_err_ef, batch=dephasing_batch_num_w_noise,
                                 label="Dynamical Decoupling + EF Noise"+extra_label, color="orange", marker="^"),
        }
        inner_key=2

        fields = ("dt", "y", "err", "batch")
        common = set.intersection(*(set(d["batch"][inner_key]) for d in DATASETS.values()))

        # ── 2. drop any row whose batch is *not* in that intersection
        for d in DATASETS.values():
            keep = [b in common for b in d["batch"][inner_key]]  # True/False mask

            # apply the same mask to every sub-list of every field
            for fld in fields:
                for k, sublist in d[fld].items():  # k = 0 … 5
                    d[fld][k] = [v for v, ok in zip(sublist, keep) if ok]

            # optional sanity check (remove for speed)
            lens = {len(d[fld][inner_key]) for fld in fields}
            if len(lens) != 1:
                raise ValueError("length mismatch after trimming")
        ...
        for q, ax in enumerate(axes):
            ...
            for name, meta in DATASETS.items():
                if name == "t1":  # skip the T1 dataset
                    continue
                if meta["dt"] is None:  # dataset absent → skip
                    continue
                if q >= len(meta["dt"]):  # protects against ragged dicts
