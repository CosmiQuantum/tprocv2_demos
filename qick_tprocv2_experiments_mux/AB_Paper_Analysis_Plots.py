...
################################### Definitions of additional plotting funcs ####################################
...
def boxwhisker_t1t2_per_qubit_vs_run(
    ...
    if mode.lower() == "together":
        ...
        #fig.suptitle(fig_title_prefix, fontsize=fontsize) # temporarily removing title
    ...
    # ------------------------- mode: separate (one figure per metric) -------------------------
        ...
        for (label, vals_by_run, color) in metric_specs:
            ...
            # fig.suptitle(f"{label}{fig_title_prefix}", fontsize=fontsize) # temporarily removing title

            # Centering x-axis label with the second column
            middle_ax = axes[1]  # top-middle subplot
            bbox = middle_ax.get_position()
            x_center = (bbox.x0 + bbox.x1) / 2

            fig.supxlabel("Run Number",fontsize=fontsize,x=x_center)
            ...
            if save_plt_path is None:
                ...
                fname = os.path.join(save_plt_path,f"boxwhisk_{label}_vs_run_num_{timestamp}.pdf")
...
def boxwhisker_qtemps_per_qubit_vs_run_choice(
    ...
    if layout == "separate":
        ...
        for ax_idx, q in enumerate(qubits_to_plot):
            ...
            if multi_qubit_colors:
                if plot_mode == "hybrid":
                    ...
                    legend_handles = [
                        Patch(facecolor=q_color, edgecolor=q_color, alpha=0.30, label="RPM"),
                        Patch(facecolor=ssf_color, edgecolor=ssf_color, alpha=0.30, label="SSF")
                    ]
        ...
        #fig.suptitle(fig_title, fontsize=suptitle_fs, y=0.97) # temporarily commented out the fig title
        ...
        if plot_mode == "compare_methods":
            legend_handles = [
                Patch(facecolor=colors[0], edgecolor=colors[0], alpha=0.30, label="RPM"),
                Patch(facecolor=ssf_color, edgecolor=ssf_color, alpha=0.30, label="SSF")]
            ...
            #fig.suptitle(compare_title, fontsize=suptitle_fs, y=1.05) # commenting out title temporarily for paper
...
def boxwhisker_pe_per_qubit_vs_run_hybrid(
    ...
    for ax_idx, q in enumerate(qubits_to_plot):
        ...
        if log_y:
            ...
            # Log scale cannot include zero (we should not get this type of data anyways though)
            # if ymin <= 0:
            #     ymin = 0.1  # percent
    ...
    legend_handles = [
        Patch(
            ...
            edgecolor=ssf_color,
            alpha=0.31,
            label="Run 2 (SSF)" # "Run 5 (SSF Method)"
        ),
        Patch(
            facecolor=rpm_legend_color,
            edgecolor=rpm_legend_color,
            alpha=0.31,
            label="Runs 3-6 (RPM)" # "Runs 6-9 (RPM Method)"
        )]
    ...
    if 2 in qubits_to_plot:
        ...
        fig.legend(
            handles=legend_handles,
            loc="lower right",
            bbox_to_anchor=(0.64, -0.07), # (0.96, 0.3) for 5-qubits case only
            fontsize=label_fs - 2,
            frameon=True)

    #fig.suptitle(fig_title, fontsize=suptitle_fs, y=0.97) # temporarily commenting out fig title
...
def boxwhisker_ssf_per_qubit_vs_run(
    ...
    if save_plt_path is None:
        ...
        fig.savefig(fname, bbox_inches="tight")
        plt.close(fig)
def boxwhisker_snr_per_qubit_vs_run(
        run_num_list,
        snr_vals_by_run,
        n_qubits=6,
        qubits_to_plot=None,
        colors="navy",
        # default: same color for all qubits, can pass multiple too:
        # ('orange', 'blue', 'purple', 'green', 'brown', 'palevioletred')
        ylims=None,
        yticks=None,
        showfliers=True,
        whis=1.5,
        ...
        tick_fs=18,
        save_plt_path=None,
        save_name="boxwhisk_snr_vs_run_num.pdf"
):
    """
    SNR boxplot, using already-grouped per-run dictionaries.

    Expected input shape
    --------------------
    snr_vals_by_run[run][q] = [snr, snr, ...]

    Notes
    -----
    - This is the SNR-only version of the SSF boxplot function.
    - Default color is navy for all qubits.
    - If you want different colors per qubit, pass something like:
          colors=('orange', 'blue', 'purple', 'green', 'brown', 'palevioletred')
    """
    ...
    for ax_idx, q in enumerate(qubits_to_plot):
        ...
        box_data = [cell_to_1d(get_cell(snr_vals_by_run, r, q)) for r in run_num_list]
