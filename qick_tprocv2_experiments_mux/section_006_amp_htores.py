...
class Htores_AmplitudeRabiExperiment:
    def __init__(self, QubitIndex, number_of_qubits, outerFolder, round_num, signal, save_shots=False, save_figs = True, experiment = None,
                 ...
                 projective_readout_pulse_len_us=9,  time_between_projective_readout_pulses=None, expt_name = "power_rabi_htores", unmasking_resgain = False):
        ...
        if unmasking_resgain:
            ...
            if experiment is not None:
                ...
                print(f'Q {self.QubitIndex + 1} Round {self.round_num} Htores Rabi configuration: ', self.config)
        ...
        try:
            ...
            if config is not None:
                fig.text(plot_middle, 0.98,
                         f"Htores Rabi Q{self.QubitIndex + 1}: {pi_amp:.4f} (a.u.) _"  + f", {config['reps']}*{config['rounds']} avgs",
                ...
                fig.text(plot_middle, 0.98,
                         f"Htores Rabi Q{self.QubitIndex + 1}: {pi_amp:.4f} (a.u.)_" f", {self.config['sigma'] * 1000} ns sigma" + f", {self.config['reps']}*{self.config['rounds']} avgs",
...
class FH_AmplitudeRabiProgram(AveragerProgramV2):
    def _initialize(self, cfg):
        ...
        self.add_gauss(ch=qubit_ch, name="ramp", sigma=cfg['sigma_htores'], length=cfg['sigma_htores'] * 4, even_length=False)
