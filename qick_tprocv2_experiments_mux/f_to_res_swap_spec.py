...
class FtoResQubitSpectroscopy:
    def __init__(self, QubitIndex, number_of_qubits, list_of_all_qubits, outerFolder,  round_num, signal, save_figs, experiment = None, live_plot = None):
        ...
        self.expt_name = "qubit_spec_ftores"
        self.signal = signal
        ...
        if experiment is not None:
            ...
            print(f'Q {self.QubitIndex + 1} Round {self.round_num} FtoRes Qubit Spec configuration: ', self.config)

    def run(self, soccfg, soc):
        qspec = FtoResPulseProbeSpectroscopyProgram(soccfg, reps=self.config['reps'], final_delay=0.5, cfg=self.config)
    ...
    def plot_results(self, I, Q, freqs, config=None, fig_quality=100):
        ...
        #              f"FtoRes Qubit Spectroscopy Q{self.QubitIndex + 1}, %.2f MHz" % largest_amp_curve_mean +
...
class FtoResPulseProbeSpectroscopyProgram(AveragerProgramV2):
    ...
    def _body(self, cfg):
        ...
        self.delay_auto(t=0.0, tag='waiting after pi_ge')  # Wait til qubit pulse is done before proceeding
        self.pulse(ch=self.cfg["qubit_ch"], name="pi_ef", t=0)  # play ge pi pulse
        self.delay_auto(t=0.0, tag='waiting after pi_ef')  # Wait til qubit pulse is done before proceeding

        self.pulse(ch=self.cfg["qubit_ch"], name="qubit_pulse", t=0)  # play probe pulse
        self.delay_auto(t=0.0, tag='waiting')  # Wait til qubit pulse is done before proceeding
