...
for i, ax in enumerate(axes):
    ...
    if i not in date_times_t1 or i not in t1_vals:
    ...
    if len( date_times_t1[i])>1:
        date_label = date_times_t1[i][0]
    else:
        date_label = ''


    if len(t1_vals[i]) >1:
        optimal_bin_num = 30 #optimal_bins(t1_vals[i])
...
plt.tight_layout()
plt.savefig( analysis_folder + 'T1hists.png', transparent=False, dpi=100)
