...
class T2RMeasurement:
    ...
    def t2_fit(self, x_data, I, Q, verbose=False, guess=None, plot=False):
        ...
        # Kesters function
        def func(t, A, B, T2, phi, f1, f2, c):
            return (np.exp(-t / (T2)) * (
                    A * np.sin((2 * np.pi * f1 * t) + phi) + B * np.sin((2 * np.pi * f2 * t) + phi))) + c
        ...
        f2g = 1.25  # MHz
        cg = 1.1
        guess = [Ag, Bg, T2g, phig, f1g, f2g, cg]
        ...
        if plot:
            ...
            f2 = popt[5]  # MHz
            c = popt[6]
            plt.plot(t, func(t, A, B, T2, phi, f1, f2, c), label='fit')  # Kester fit
            # plt.plot(x_data, fit_type(x, popt) * y_normal) # QUA library fit
            #     plt.plot(
            #         x_data,
            #         y_data,
            #         ".",
            ...
            # plt.plot(xdata, func(t,Ag, T2g,phig,f1g,f2g, cg), label='guess')
            plt.plot(t, func(t, A, B, T2, phi, f1, f2, c), label='fit')

        return fit_type(x, popt) * y_normal, T2, t2r_err, plot_sig
        ...
        if self.fit_data:
            ...
            if config is not None:
                fig.text(plot_middle, 0.98,
                         f'T2={round(T2, 2)}us while biased at 0.075usV\n exp(-t/{round(T2, 2)})[{round(A, 2)}sin({round(2 * np.pi * f1, 2)}t + {round(phi, 2)}) + {round(B, 2)}sin({round(2 * np.pi * f2)}t + {round(phi, 2)})] + {round(c, 2)}\n $\Delta$f1={round(f1, 2)}MHz and $\Delta$f2={round(f2, 2)}MHz',
