...
Qs_to_look_at = [0, 1, 2, 3, 4, 5]  # # only process these qubits

# Set which experiments to run
run_flags = {"q_spec": True, "rabi": False, "ss": True, "t1": False}

# Optimization parameters for resonator spectroscopy
res_leng_vals = [4.3, 5.1, 3.8, 6, 5.7, 21]
...
# Configure Logging
################################################
outerFolder = os.path.join("/data/QICK_data/6transmon_run6/", 'angle_test')
...
################################################
def create_data_dict(keys, qs):
    return {Q: {key: np.empty(1, dtype=object) for key in keys} for Q in range(len(qs))}
...
def tune_qubits():
    ...
    for QubitIndex in Qs_to_look_at:
        ...
        qubitFolder = os.path.join("/data/QICK_data/6transmon_run6/", 'angle_test')
...
def fast_RR_loop(stored_qspec_list, res_freqs_list, pi_amp_list):
    ...
    qspec_data = create_data_dict(qspec_keys, list_of_all_qubits)
    rabi_data = create_data_dict(rabi_keys,  list_of_all_qubits)
    ss_data = create_data_dict(ss_keys, list_of_all_qubits)
    t1_data = create_data_dict(t1_keys, list_of_all_qubits)
    j = 0
    while j < n:
        for QubitIndex in Qs_to_look_at:
            batch_num = 0

            qubitFolder = os.path.join("/data/QICK_data/6transmon_run6/", f'angle_test')
            ...
            if run_flags["q_spec"]:
                ...
                experiment.qubit_cfg['qubit_freq_ge'][QubitIndex] = float(stored_qspec_list[QubitIndex])
            ...
            if run_flags["rabi"]:
                ...
                experiment.qubit_cfg['pi_amp'][QubitIndex] = float(pi_amp_list[QubitIndex])
            ...
            try:
                if save_data_h5:
                    ...
                    if run_flags["ss"]:
                        ...
                        ss_data[QubitIndex]['Batch Num'][0] = batch_num
                    ...
                    # Save data every '1' rounds
                    if j % 1 == 0:
                        batch_num += 1
                        if run_flags["q_spec"]:
                            saver_qspec = Data_H5(qubitFolder, qspec_data, batch_num, 1)
                            saver_qspec.save_to_h5('QSpec')
                            del saver_qspec
                            gc.collect()
                        if run_flags["rabi"]:
                            saver_rabi = Data_H5(qubitFolder, rabi_data, batch_num, 1)
                            saver_rabi.save_to_h5('Rabi')
                            del saver_rabi
                            gc.collect()
                        if run_flags["ss"]:
                            saver_ss = Data_H5(outerFolder, ss_data, batch_num, 1)
                            saver_ss.save_to_h5('SS')
                            del saver_ss
                            del ss_data

                        if run_flags["t1"]:
                            saver_t1 = Data_H5(qubitFolder, t1_data, batch_num, 1)
