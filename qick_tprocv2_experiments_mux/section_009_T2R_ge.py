    ...
    def run(self, thresholding=False, use_iminuit_instead = True):
        ...
        if self.fit_data:
            if use_iminuit_instead:
                fit, t2r_est, t2r_err, plot_sig, out = self.t2_fit_iminuit(delay_times, I, Q)
            else:
                fit, t2r_est, t2r_err, plot_sig = self.t2_fit(delay_times, I, Q) # curve fit
        else:
        ...
        if self.fit_data:
            ...
            if config is not None:
                fig.text(plot_middle, 0.98,
                         f"Q{self.QubitIndex + 1} T2R {t2r_est:.2f} +/- {t2r_err:.2f}us" + f", {float(config['reps'])}*{float(config['rounds'])} avgs,",
                         fontsize=24, ha='center', va='top')
            else:
                fig.text(plot_middle, 0.98,
                         f"Q{self.QubitIndex + 1} T2R {t2r_est:.2f} +/- {t2r_err:.2f}us, {float(self.config['reps'])}*{float(self.config['rounds'])} avgs,",
                         fontsize=24, ha='center', va='top')
            ...
            if config is not None:
                ...
                fig.text(plot_middle, 0.98,
                         f"T2R Q{self.QubitIndex + 1}, {float(self.config['reps'])}*{float(self.config['rounds'])} avgs,",
                         fontsize=24, ha='center', va='top')
