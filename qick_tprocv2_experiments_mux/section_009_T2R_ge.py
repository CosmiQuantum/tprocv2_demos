...
class T2RMeasurement:
    ...
    def t2_fit(self, x_data, I, Q, verbose = False, guess=None, plot=False):
        # This fitting code was adapted from:
        # https://github.com/qua-platform/py-qua-tools/blob/37c741ade5a8f91888419c6fd23fd34e14372b06/qualang_tools/plot/fitting.py
        if abs(I[-1] - I[0]) > abs(Q[-1] - Q[0]):
            y_data = I
            plot_sig = 'I'
        else:
    ...
    def run(self, thresholding=False):
        ...
        if self.fit_data:
            fit, t2r_est, t2r_err, plot_sig = self.t2_fit_iminuit(delay_times, I, Q)
        else:
