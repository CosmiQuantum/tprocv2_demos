...
import jcresonators.functions as ff
...
def PlotFit(x,
    ...
    #     plt.gcf().text(0.05, 0.9, plot_title, fontsize=15)
    # if len(title) > 40:
    #     plot_title = title[0:40] + "\n" + title[40:]
    #     plt.gcf().text(0.05, 0.9, plot_title, fontsize=15)
    # else:
    #     plt.gcf().text(0.05, 0.92, title, fontsize=15)
 ...
 #       tick.label.set_fontsize(fsize)
    ...
    try:
        if params != []:
            if func == ff.cavity_inverse:
                ...
                #plt.gcf().text(0.5, 0.11, textstr, fontsize=10)
            elif func == ff.cavity_CPZM:
                if params[0] < 0:
                    print(
                        "Qi is less than zero. Please make sure data is of correct format: decibals (log10*20 version), and radians. Otherwise, it is quite likely that the resonator being fit is not a Notch type resonator. For reflection type geometry, please use DCM REFLECTION.")
                ...
                textstr = r'$Q_i$ = ' + '%s' % float('{0:.10g}'.format(Qi)) + r"$\pm$" + '%s' % float(
                    ...
                    '{0:.1g}'.format(conf_array[2])) + \
                          '\n' + r'$f_c$ = ' + '%s' % float('{0:.10g}'.format(f_c)) + r"$\pm$" + '%s' % float(
                    '{0:.1g}'.format(conf_array[3])) + ' GHz'
                #plt.gcf().text(0.5, 0.11, textstr, fontsize=18)
            else:
                ...
                if 1 / np.real(1 / Qc) < 0:
                    ...
                    #plt.gcf().text(0.5, 0.245, Qc_str, fontsize=18, color='red')
                    ...
                    textstr = 'Q = ' + '%s' % float('{0:.10g}'.format(Q)) + r"$\pm$" + '%s' % float(
                        ...
                        '{0:.1g}'.format(conf_array[4])) + ' radians' + \
                              '\n' + r'$f_c$ = ' + '%s' % float('{0:.10g}'.format(f_c)) + r"$\pm$" + '%s' % float(
                        '{0:.1g}'.format(conf_array[5])) + ' GHz'
                    #plt.gcf().text(0.6, 0.09, textstr, fontsize=14)
    ...
    parameters = np.array([Q, Qi, Qc, Qc_Re, phi, f_c])
    return x, y, x_fit, y_fit, parameters
...
def fit(resonator):
    ...
    if Method.method == 'DCM':
        ...
        x, y, xfit, yfit, parameters = PlotFit(x_raw, y_raw, x_initial, y_initial, slope, intercept, slope2, intercept2, output_params, Method,
        ...
        try:
            ...
            x, y, xfit, yfit, parameters = PlotFit(x_raw, y_raw, x_initial, y_initial, slope, intercept, slope2, intercept2, output_params,
        ...
        try:
            ...
            x, y, xfit, yfit, parameters = PlotFit(x_raw, 1 / y_raw, x_initial, y_initial, slope, intercept, slope2, intercept2, output_params,
    ...
    return x, y, xfit, yfit, parameters
