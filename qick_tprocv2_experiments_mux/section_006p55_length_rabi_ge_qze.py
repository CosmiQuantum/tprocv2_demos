...
class LengthRabiExperimentQZE:
    ...
    def run_QZE_one_starked_qfreq(self, wait_for_res_ring_up=False,
                                  ...
                                  turn_pulse_off_after_T1=False, check_t1_with_qubit_pulse_on=False,
                                  check_t1_with_res_pulse_on=False):
        ...
        for row_idx, length in enumerate(qubit_length_ge_loop):
            ...
            elif turn_pulse_off_after_T1:
                amp_rabi = QZERabiT1AfterPulseTurnedOff(
                    self.experiment.soccfg,
                    reps=updated_config['reps'],
                    final_delay=updated_config['relax_delay'],
                    cfg=updated_config
                )
            elif check_t1_with_qubit_pulse_on:
                amp_rabi = QZERabiT1withQubitPulseOn(
                    self.experiment.soccfg,
                    reps=updated_config['reps'],
                    final_delay=updated_config['relax_delay'],
                    cfg=updated_config
                )
            elif check_t1_with_res_pulse_on:
                amp_rabi = QZERabiT1withResPulseOn(
                    self.experiment.soccfg,
                    reps=updated_config['reps'],
                    final_delay=updated_config['relax_delay'],
                    cfg=updated_config
            ...
            # get the lengs that were used so you can use to plot on the x axis
            lengths.append(length)#amp_rabi.get_pulse_param('qubit_pulse', "length")) #should be just a float by default
...
class QZERabiT1AfterPulseTurnedOff(AveragerProgramV2):
    def _initialize(self, cfg):
        ro_ch = cfg['ro_ch']
        res_ch = cfg['res_ch']
        qubit_ch = cfg['qubit_ch']
        ...
        # projection pulse temporary
        if cfg['qubit_length_ge'] > cfg['qubit_pi_len']:
            self.add_pulse(ch=res_ch, name="proj_pulse_tmp",
                           style="const",
                           length=cfg['finish_qze_pulse_length'] - cfg['qubit_pi_len'] + cfg['res_ring_up_time'] , #subtract the qubit length minus stop lenght time
                           mask=cfg["qze_mask"],
                           )
        self.add_pulse(ch=res_ch, name="proj_pulse_no_qubit_drive",
                       style="const",
                       length=cfg['finish_qze_pulse_length'], #stopped qubit pulse at 100us
                       mask=cfg["qze_mask"],
                       )
        ...
        if cfg['finish_qze_pulse_length'] >= cfg['qubit_length_ge'] > cfg['qubit_pi_len']+0.01:
            self.add_pulse(ch=qubit_ch, name="starked_qubit_pulse",
                           style="const",
                           length=cfg['qubit_length_ge']-cfg['qubit_pi_len'],  # total_drive_length,
        ...
        if cfg['qubit_length_ge'] > cfg['finish_qze_pulse_length']:
            self.add_pulse(ch=qubit_ch, name="starked_qubit_pulse_tmp",
                           style="const",
                           length=cfg['finish_qze_pulse_length'] - cfg['qubit_pi_len'],  # total_drive_length,
                           freq=cfg['qubit_freq_ge_starked'],  # _starked
                           phase=cfg['qubit_phase'],
                           gain=cfg['qubit_gain_ge'])
    ...
    def _body(self, cfg):
        #drive the qubit on the qubit channel (list with length 6)
        print("cfg['qubit_pi_len']: ", cfg['qubit_pi_len'], "cfg['qubit_length_ge']: ", cfg['qubit_length_ge'])
        if cfg['qubit_length_ge'] <= cfg['qubit_pi_len']:
            ...
            self.delay_auto(t=0, tag='waiting_pi')  # wait to finish
            self.pulse(ch=cfg['res_ch'], name="proj_pulse_tmp", t=0)  # play zeno/stark tone in resonator
            self.pulse(ch=cfg["qubit_ch"], name="starked_qubit_pulse_tmp", t=cfg[
                'res_ring_up_time'])  # play starked freq qubit drive for rest of qubit pulse len,, start 3 us after ring up

            self.delay_auto(t=0, tag='waiting')  # auto wait for those pulses to be done
            #wait for ring up time and extra time after the pulses finish
            self.delay(t=cfg['res_ring_up_time']+ cfg['qubit_length_ge'] - cfg['finish_qze_pulse_length'])  # wait for ring down that will happen in qze, but of course the resonator doesnt actually have photons here

        #trigger the readout resonator pulse (9us long).
        self.pulse(ch=cfg['res_ch'], name="res_pulse")
        self.trigger(ros=cfg['ro_ch'], pins=[0], t=cfg['trig_time'])

class QZERabiT1withQubitPulseOn(AveragerProgramV2):
    def _initialize(self, cfg):
        ro_ch = cfg['ro_ch']
        res_ch = cfg['res_ch']
        qubit_ch = cfg['qubit_ch']
        ...
        if 100 >= cfg['qubit_length_ge'] > cfg['finish_qze_pulse_length']:
            self.add_pulse(ch=qubit_ch, name="starked_qubit_pulse_tmp",
                           style="const",
                           length=cfg['qubit_length_ge'] - cfg['qubit_pi_len'],  # total_drive_length,
                           freq=cfg['qubit_freq_ge_starked'],  # _starked
        ...
        if cfg['qubit_length_ge'] <= cfg['qubit_pi_len']+0.01:
            ...
            if cfg['qubit_length_ge'] > 100:
                for num in range(math.floor(cfg['qubit_length_ge'] / 100)):
                    self.pulse(ch=cfg["qubit_ch"], name="starked_qubit_pulse_100",
                               t=cfg['res_ring_up_time'] + num * 100)  # play pulse

                self.pulse(ch=cfg["qubit_ch"], name="starked_qubit_pulse_remainder",
                           t=cfg['res_ring_up_time'] + (math.floor(cfg['qubit_length_ge'] / 100))* 100)  # play remainder ofpulse
            else:
                self.pulse(ch=cfg["qubit_ch"], name="starked_qubit_pulse_tmp", t=cfg['res_ring_up_time'])
            self.delay_auto(t=0, tag='waiting')  # auto wait for those pulses to be done
...
class QZERabiT1withResPulseOn(AveragerProgramV2):
    def _initialize(self, cfg):
        ro_ch = cfg['ro_ch']
        res_ch = cfg['res_ch']
        qubit_ch = cfg['qubit_ch']
        ...
        if cfg['qubit_length_ge'] > cfg['qubit_pi_len']:
            self.add_pulse(ch=res_ch, name="proj_pulse_tmp",
                           style="const",
                           length=cfg['qubit_length_ge'] - cfg['qubit_pi_len'] + cfg['res_ring_up_time'],
                           # subtract the qubit length minus stop lenght time
                           mask=cfg["qze_mask"],
                           )
        self.add_pulse(ch=res_ch, name="proj_pulse_no_qubit_drive",
                       style="const",
        ...
        self.add_pulse(ch=qubit_ch, name="qubit_pulse_pi_len",
                       # after pi pulse len, this is the normal non zeno freq drive
                       style="const",
                       length=cfg['qubit_pi_len'],  # total_drive_length,
                       freq=cfg['qubit_freq_ge'],  # [0] # only should be one value
                       phase=cfg['qubit_phase'],
    ...
    def _body(self, cfg):
        ...
        if cfg['qubit_length_ge'] <= cfg['qubit_pi_len'] + 0.01:
            ...
            self.delay_auto(t=0, tag='waiting_pi')  # wait to finish
            self.pulse(ch=cfg['res_ch'], name="proj_pulse", t=0)  # play zeno/stark tone in resonator
            self.pulse(ch=cfg["qubit_ch"], name="starked_qubit_pulse", t=cfg[
                'res_ring_up_time'])  # play starked freq qubit drive for rest of qubit pulse len,, start 3 us after ring up
            self.delay_auto(t=0, tag='waiting')  # auto wait for those pulses to be done
            self.delay(t=cfg['res_ring_up_time'])
