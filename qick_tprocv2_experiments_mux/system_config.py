...
class QICK_experiment:
    def __init__(self, folder, DAC_attenuator1 = 10, DAC_attenuator2 = 11, qubit_DAC_attenuator1 = 2,
                                     qubit_DAC_attenuator2 = 2, ADC_attenuator = 17, fridge = None):
        if fridge == "QUIET":
            ...
            self.soc.rfb_set_gen_filter(self.FSGEN_AMPL_CH, fc=self.qubit_center_freq / 1000, ftype='bandpass', bw=1.7)
            ...
            self.readout_cfg = {
                ...
                'res_freq_ge': np.array([6227.187, 6289.150, 6348.55, 6419.665, 6485.315, 6552.35]), #updated by Arianna 10/10, run 8
                'rqubit_freq_ge': np.array([6227.187, 6289.175, 6348.55, 6419.665, 6485.315, 6552.35]),
                "meas_freq_ge": np.array([6227.187, 6289.175, 6348.55, 6419.665, 6485.315, 6552.35]),  # Arianna 10/10, run 8
                # 'qubit_freq_ge': np.array([6227.187, 6289.175, 6348.55, 6419.665, 6485.315, 6552.35]),
                #'res_freq_ge': [6223.097, 6284.55, 6343.95, 6414.934, 6481.4, 6547.250],  # run 7
                "rqubit_gain_ge": [0.04, 1, 0.05, 0.08, 0.14, 0.17],
                ...
                "meas_gain_ge": [0.95, 0.9, 0.95, 0.55, 0.55, 0.95],  # [1, 1, 1, 1, 1, 1],
                ...
                "res_freq_ef": [6227.212, 6289.074, 6348.438, 6419.583, 6485.360, 6547.128],  # [MHz] updated by arianna for run 8, 10/2
                ...
                # "meas_length_ge": 6.5,
                "meas_length_ge": [2.0] * 6,  # [us] (1.0 for res spec)
                "res_lengths": [2.0]*6,  # [us] (1.0 for res spec)
                "res_phase": [0,180+((2.0)/np.pi)*180,0,0,0,0],#[ -180+((1.281174-2.6703) * 180/np.pi), -10, 85,
                            #   0, 150,
                            # -90], #Joyce 3/11
                "res_phase_fh": [0, ((2.0) / np.pi) * 180, 0, 0, 0, 0],
                ...
                "meas_phase": [180 + ((2.8) / np.pi) * 180] * 6,  # [deg]
                ...
                "qubit_is_in_g_threshold": 14.4, #100000,#-8837,
                "edge_of_e_state_threshold": 80,
                "mid_threshold": -370,
                "edge_of_h_state_threshold": 20,
                "edge_of_f_state_threshold": -420,
                'qubit_freq_ge': np.array([6227.187, 6289.175, 6348.55, 6419.665, 6485.315, 6552.35]),
                "qubit_gain_ge": [0.04, 1, 0.05, 0.08, 0.14, 0.17],
                "rqubit_phase": [180 + ((2.8) / np.pi) * 180] * 6,  # [deg]
            }

            # Qubit Configuration
            self.qubit_cfg = {
                "qubit_freq_ge": [4194.71, 3828.58, 6289.175, 4474.23, 4485.38, 5018.12], # Arianna 10/10, run 8
                # 'qubit_freq_ge': np.array([6227.187, 6289.175, 6348.55, 6419.665, 6485.315, 6552.35]),
                # 'rqubit_freq_ge': np.array([6227.187, 6289.175, 6348.55, 6419.665, 6485.315, 6552.35]),
                # 'rqubit_freq_ge': np.array([6227.187, 6289.175, 6348.55, 6419.665, 6485.315, 6552.35]),
                "meas_freq_ge": [6227.187, 6289.175, 6348.55, 6419.665, 6485.315, 6552.35],  # Arianna 10/10, run 8
                ...
                "qubit_gain_ge":  [0.005, 0.0001, 0.005, 0.005, 0.005, 0.005],
                # "rqubit_gain_ge": [0.04, 1, 0.05, 0.08, 0.14, 0.17],
                # "rqubit_gain_ge": [0.04, 1, 0.05, 0.08, 0.14, 0.17],
                "meas_gain_ge": [0.04, 1.0, 0.05, 0.08, 0.14, 0.17],
                ...
                "qubit_length_ge": 22,  # 15 [us] for spec Pulse
                "rmeas_length_ge": 5,  # 15 [us] for spec Pulse
                "qubit_freq_ef": [4020.47, 3650.79, 3999.01, 4302.33, 4313.20, 4848.81], # Arianna 10/13
                # [MHz] Freqs of Qubit e/f Transition
                "qubit_freq_fh": [3831.73, 3456.10625, 3988.44, 4292.73, 4292.73, 4833.17],
                "qubit_freq_eh" : [4016.3, 3450.78, 3988.44, 4292.73, 4292.73, 4833.17],
                ...
                "qubit_gain_ef":  [0.0055, 0.01, 0.0077, 0.0045, 0.0095, 0.02],# [0.004, 0.0075, 0.0045, 0.008, 0.01],#
                "qubit_gain_fh": [0.00005, 0.0005, 0.0075, 0.1, 0.15, 0.005],
                "qubit_gain_eh": [0.001, 1, 0.0075, 0.1, 0.15, 0.005],
                'qubit_gain_ftores': [1]*6,#[0.2, 0.14, 0.04, 0.17, 0.13, 0.08],
                'qubit_gain_htores': [0.5] * 6,  # [0.2, 0.14, 0.04, 0.17, 0.13, 0.08],
                # [0.01, 0.05, 0.05, 0.05, 0.01, 0.5], # [DAC units] Pulse Gain
                "qubit_length_ef": 22, #22.0,
                "qubit_length_fh": 60,  # 22.0,
                ...
                "sigma": [0.2, 0.25, 0.2, 0.2, 0.2, 0.2],  # DAC 2 04/07 [us] for Gaussian Pulse (5+10 DAC atten for qubit). Updated on 7/17 by Arianna
                ...
                "sigma_ef": [0.1, 0.25, 0.2, 0.1, 0.2, 0.2],  # [us] for Gaussian Pulse, #Arianna 3/27
                "sigma_fh": [0.2, 0.15, 0.2, 0.2, 0.2, 0.2],  # [us] for Gaussian Pulse, #Arianna 3/27
                "pi_amp": [0.655, 0.568, 0.65, 0.65, 0.66, 0.69], # Arianna 10/10, run 8
                ...
                "pi_ef_amp": [0.427, 0.6082, 0.511, 0.7018, 0.6751, 0.589], # Arianna 3/27
                "pi_fh_amp": [0.563, 0.5629, 0.511, 0.7018, 0.6751, 0.589],  # Arianna 3/27
                "qubit_mixer_freq": 4000,  # [MHz]
                "qubit_mixer_freq2": 6000,  # [MHz]
                # "mixer_freq": 6000,  # [MHz]
