...
# Filesystem inputs
runFolder = '/data/QICK_data/run6'
deviceFolder = '6transmon'
studyFolder = 'Fast_Relaxation_Measurements'
substudyFolder = 'Preliminary'
substudy_txt_notes = "test"
...
# Keys defining the data dictionaries in this
...
##-----------------------------------------------------------------------------------------------------------
## Create a directory tree for the high-level directories needed for data organization. This runs first, and
## will create requisite junkyard and substudy folders, but not the dataset folders, which are timestamped
## according to when they're actually run.
def create_directory_tree():
    print('--fast_relaxation_measurements_with_optimization: Creating Directory Tree.')
    ...
    ##First, check to see if the run folder exists. If not, add it
    tempRunFolder = runFolder
    if not (os.path.exists(runFolder)):
        os.makedirs(runFolder)

    ##Next, check to see if the device folder exists. If not, add it
    tempDeviceFolder = os.path.join(runFolder,deviceFolder)
    if not (os.path.exists(tempDeviceFolder)):
        os.makedirs(tempDeviceFolder)

    ##Next, check to see if the study folder exists
    tempStudyFolder = os.path.join(runFolder,deviceFolder,studyFolder)
    if not (os.path.exists(tempStudyFolder)):
        os.makedirs(tempStudyFolder)

    ##Next, check to see if the study's Junkyard folder exists
    tempJunkyardFolder = os.path.join(runFolder,deviceFolder,studyFolder,'Junkyard')
    if not (os.path.exists(tempJunkyardFolder)):
        os.makedirs(tempJunkyardFolder)
        
    ##Next, check to see if the substudy folder exists
    tempSubStudyFolder = os.path.join(runFolder,deviceFolder,studyFolder,substudyFolder)
    if not (os.path.exists(tempSubStudyFolder)):
        os.makedirs(tempSubStudyFolder)
        
    ##Next, write the substudy text notes
    if not (os.path.exists(os.path.join(tempSubStudyFolder,'sub_study_notes.txt'))):
        file_path = os.path.join(tempSubStudyFolder, 'sub_study_notes.txt')
        with open(file_path, "w", encoding="utf-8") as file:
            file.write(substudy_txt_notes)
        
    #Package these into a dictionary that can be used in the rest of the functions
    output = {"run" : tempRunFolder, "device" : tempDeviceFolder, "study" : tempStudyFolder, "substudy" : tempSubStudyFolder, "junkard" : tempJunkyardFolder}
    return output


##-----------------------------------------------------------------------------------------------------------
## Create a directory tree for the specific dataset now being run. This will create a timestamp dataset folder
## within which there is an optimization and study sub-folder optimization and study
def create_dataset_directory_tree( studyDirectoryTree ):

    print('--fast_relaxation_measurements_with_optimization: Creating Dataset Directory Tree.')
    print('--fast_relaxation_measurements_with_optimization: substudy is {}'.format(studyDirectoryTree["substudy"]))
    
    ##First, we identify a timestamped name for the dataset, and then use it to create the other 
    formatted_datetime = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    datasetFolder = os.path.join(studyDirectoryTree["substudy"],formatted_datetime)
    print(datasetFolder)
    print(studyDirectoryTree["substudy"])
    if not (os.path.exists(datasetFolder)):
        os.makedirs(datasetFolder)

    ##Next, do sub-folders: optimization
    ...
    ##Next, do sub-folders: study data
    studyDataFolder = os.path.join(datasetFolder, 'study')
    if not (os.path.exists(studyDataFolder)):
        os.makedirs(studyDataFolder)

    ##Next, do sub-folders: documentation
    ...
    ##Next, save study notes
    studyNotesPath = os.path.join(studyDocumentationFolder,'dataset_notes.txt')

    output = {"dataset" : datasetFolder, "optimization":optimizationFolder, "studyData":studyDataFolder,"studyDocumentation":studyDocumentationFolder, "datasetNotes": studyNotesPath}
    return output

##-----------------------------------------------------------------------------------------------------------
## This cleanly sets up logging for this fast relaxation measurement. We return the logger for use by other functions
## Arguments:
## 1. Directory tree down to the substudy level (output of create_directory_tree)
def SetupLogger( studyDirectoryTree ):
    log_file = os.path.join(studyDirectoryTree['substudy'], "RR_TLS_script.log")
    frm_logger = logging.getLogger("custom_logger_for_frm_only")
    frm_logger.setLevel(logging.DEBUG)
    file_handler = logging.FileHandler(log_file, mode='a')
    formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
    file_handler.setFormatter(formatter)
    frm_logger.addHandler(file_handler)
    frm_logger.propagate = False
    return frm_logger

    

##-----------------------------------------------------------------------------------------------------------
## This is the optimization block, the bit that goes into the optimization subfolder of the dataset, and which
## involves basic measurements like res spec, qu spec, etc.
## Arguments:
## 1. The dataset (lower) directory tree, so we know here things go
## 2. Here, the qubit index, since we're not yet multiplexing
## 3. The experiment object
def RunSingleQubitOptimization( datasetDirectoryTree, QubitIndex, experiment, frm_logger ):
    ...
    ################################################ Res Spec ################################################
    #Create a container for resonator spec data. This will include space for all qubits even though only one is
    #being accessed here
    res_data = create_data_dict(res_keys, save_r, list_of_all_qubits)
    res_freqs_samples = []

    #Loop over resonator spectroscopy iterations
    for sample in range(res_sample_number):        
        try:
            res_spec = ResonanceSpectroscopy(QubitIndex, tot_num_of_qubits, datasetDirectoryTree['optimization'], sample,
                                             save_figs, experiment=experiment, verbose=verbose,
                                             logger=frm_logger, qick_verbose=qick_verbose)
            res_freqs, freq_pts, freq_center, amps, sys_config_rspec = res_spec.run()
            res_freqs_samples.append(res_freqs)
            frm_logger.info(f"ResSpec sample {sample} for qubit {QubitIndex}: {res_freqs}")
            ...
            del res_spec
            print('--fast_relaxation_measurements_with_optimization: successfully ran a resonator spectroscopy.')

        except Exception as e:
            frm_logger.exception(f"ResSpec error on qubit {QubitIndex} sample {sample}: {e}")
            continue
    ...
    if res_freqs_samples:
        ...
        frm_logger.error(f"No resonator spectroscopy data collected for qubit {QubitIndex}.")
        return

    experiment.readout_cfg['res_freq_ge'] = avg_res_freqs #start with offset of 0, lets try like this and see if works
    frm_logger.info(f"Avg. resonator frequencies for qubit {QubitIndex}: {avg_res_freqs}")
    with open(datasetDirectoryTree['datasetNotes'], "a", encoding="utf-8") as file:
...
## Meat of code
## Arguments:
## - QubitIndex: index of qubit being tested
## - ss_sample_number: number of single-shot measurements done prior to relaxation datataking, during cal period
## - res_sample_number: number of resonator measurements done prior to relaxation datataking, during cal period
def run_full_cycle_for_qubit(outputDirectories, QubitIndex, ss_sample_number, res_sample_number):
...
def main():
    ...
    #First, create directory tree in case it needs to be created
    studyDirectoryTree = create_directory_tree()
    frm_logger = SetupLogger(studyDirectoryTree)
    
    #Now, loop over qubits. In each case, create a new dataset directory, run optimization, and run dataset collection
    for QubitIndex in Qs_to_look_at:
        datasetDirectoryTree = create_dataset_directory_tree(studyDirectoryTree)

        #Define the experiment
        experiment = QICK_experiment( datasetDirectoryTree["dataset"],
                                      DAC_attenuator1=5,
                                      DAC_attenuator2=10,
                                      ADC_attenuator=10,
                                      fridge=FRIDGE )
        ...
        optimizationOutput = RunSingleQubitOptimization(datasetDirectoryTree,QubitIndex, experiment, frm_logger)
        #RunDatasetCollection(datasetDirectoryTree,optimizationOutput)
