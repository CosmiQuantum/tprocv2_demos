...
substudy_txt_notes = ('Initial qubit checkouts quiet run 8')#('Active Reset Test')##('round robin with relax delays of 1000us for T1 and T2 measurements.')#('Active Reset Test')#('Normal Round Robin during cooldown, now everything works properly, set debug to false to run '
...
while j < n:
    ...
    for QubitIndex in Qs_to_look_at:
        ...
        if run_flags["rabi"]:
            try:
                rabi = AmplitudeRabiExperiment(QubitIndex, tot_num_of_qubits, studyDocumentationFolder, j, signal, save_figs=save_figs,save_shots=save_shots_gerabi,
        ...
        if run_flags["ss"]:
            try:
                ...
                # ss.hist_ssf(data=[I_g, Q_g, I_e, Q_e], cfg=ss.config, plot=save_figs)
            except Exception as e:
                if debug_mode:
                    raise e # In debug mode, re-raise the exception immediately
                else:
        ...
        if run_flags["ef_Rabi"]:
            ...
            try:
                efrabi = EF_AmplitudeRabiExperiment(QubitIndex, number_of_qubits, studyDocumentationFolder, j,
                                                    signal,save_shots_efrabi, experiment=experiment, live_plot=live_plot,
                                                    increase_qubit_reps=increase_qubit_reps,
                                                    qubit_to_increase_reps_for=qubit_to_increase_reps_for,
                                                    multiply_qubit_reps_by=multiply_qubit_reps_by,
        ...
        if run_flags["ef_ss"]:
            try:
                ...
                if debug_mode:
                    ...
                    rr_logger.exception(f'Got the following error: {e}')
                    if verbose: print(f'Got the following error: {e}')
                    # we don't skip the qubit if this throws an err because we want to save the rest of the data that was taken
        ...
        if run_flags["fh_rabi"]:
            ...
            multiply_qubit_reps_by = 2  # only has impact if the line two above is True
            try:
                fhrabi = FH_AmplitudeRabiExperiment(QubitIndex, tot_num_of_qubits, studyDocumentationFolder, j, signal, save_figs=save_figs,save_shots=save_shots_fhrabi,
                                               experiment = experiment, live_plot = live_plot,
                                               increase_qubit_reps = increase_qubit_reps,
                                               qubit_to_increase_reps_for = qubit_to_increase_reps_for,
                                               multiply_qubit_reps_by = multiply_qubit_reps_by,
        ...
        if run_flags["test_act"]:
            try:
                ...
                if debug_mode:
                    ...
                    rr_logger.exception(f'Got the following error in test_act: {e}')
                    if verbose: print(f'Got the following error in test_act: {e}')
                    continue  # skip the rest of this qubit
            del act
