...
# For Quiet
substudy = "initial_optimization"#unmasking_resgain"
...
res_leng_vals  = [5.0, 5.0, 8.0, 7.0, 9.0, 7.5]#[6.5, 5.5, 7.0, 9.0, 8.5, 9.0]
# res_gain = [0.95, 0.85, 0.85, 0.55, 0.85, 0.85]
res_gain       = [1, 1, 0.85, 0.55, 0.85, 0.85]
freq_offsets   = [0, 0, -0.2273, -0.2273, -0.5000, -0.1364] # # all updated on 7/29/2025 except R5, we need to debug res spec for that resonator
...
lengs = np.arange(1, 9.5, 0.5)
start=time.time()
for QubitIndex in Qs:
    # Get the config for this qubit
    experiment = QICK_experiment(outerFolder, DAC_attenuator1=14, DAC_attenuator2=10,
                                     qubit_DAC_attenuator1=2,
                                     qubit_DAC_attenuator2=2, ADC_attenuator=17,
                                     fridge=FRIDGE)  # ADC_attenuator MUST be above 16dB #  QICK_experiment(outerFolder, DAC_attenuator1 = 10, DAC_attenuator2 = 15, qubit_DAC_attenuator1 = 5,
    ...
    optimal_lengths = [6.5, 5.5, 7.0, 9.0, 8.5, 9.0] #optional # DAC 2 optimization
    ...
    # elif QubitIndex == 1 or QubitIndex == 2  or QubitIndex == 4  or QubitIndex == 5:
    #     gain_range = [0.6, 0.85]
    # elif QubitIndex == 3:
    #     gain_range = [0.3,0.7]  # Gain range in a.u.
    # elif QubitIndex == 3:
    #     gain_range = [0.3, 0.7]
    ...
    freq_steps = int((freq_range[1] - freq_range[0])/0.1)
    gain_steps = int((gain_range[-1] - gain_range[0]) / 0.05)
