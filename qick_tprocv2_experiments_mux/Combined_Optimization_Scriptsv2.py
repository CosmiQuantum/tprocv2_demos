...
run_name ='run33e'
device_name = '4charge'
substudy = "optimization_Q4" #unmasking_resgain"
outerFolder = os.path.join(f"/home/nexusadmin/Documents/Data/{run_name}/{device_name}/Final Check/{substudy}/{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}/")
...
res_leng_vals = [3.0, 5.25, 4.5, 3.5] #4.0] #[9.5, 5.5, 6.25, 7.0] # Q1, Q4 reopt
res_gain = [0.625, 0.375, 0.475, 0.475] #[0.9, 0.7, 0.8, 0.8] # Q1, Q4 reopt
freq_offsets = [-0.25, 0.2, -0.3, 0] #[-0.1429, -0.1429, 0, -0.04] # Q1, Q2, Q4 opt r1
punch_out_vals = [0.75, 0.4, 0.51, 0.55]#[0.12, 0.1, 0.12, 0.15] # from punchout, not precise
...
lengs = np.arange(2.5, 6, 0.25) #0.1, 7, 0.5)
...
for QubitIndex in Qs:
    ...
    #             res_gains = experiment.mask_gain_res(QubitIndex, IndexGain=gain, num_qubits = number_of_qubits)
    #             experiment.readout_cfg['res_gain_ge'] = res_gains
    #
    #             ss = SingleShot(QubitIndex, number_of_qubits, outerFolder,  j, save_figs, experiment = experiment, verbose = True, unmasking_resgain = unmask)  # updated way
    ...
    optimal_lengths = [3.5, 5.25, 4.5, 3.5] #4.0] #Q1 re-optimized
