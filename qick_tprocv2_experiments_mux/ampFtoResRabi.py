...
class f2res_AmplitudeRabiExperiment:
    def __init__(self, QubitIndex, number_of_qubits, list_of_all_qubits,  outerFolder, round_num, signal, save_figs, experiment = None, live_plot = None,
        ...
        self.expt_name = "power_rabi_f2res"
        ...
        if experiment is not None:
            ...
            print(f'Q {self.QubitIndex + 1} Round {self.round_num} f2res Rabi configuration: ', self.config)
    ...
    def run(self, soccfg, soc):
        ...
        if self.live_plot:
            ...
            gains = amp_rabi.get_pulse_param('pi_f2res', "gain", as_array=True)
            print('gains', gains)
            print('I: ', I)
            print('Q: ', Q)
        # q1_fit_cosine, pi_amp = self.plot_results( I, Q, gains, config = self.config)
        self.plot_results(I, Q, gains, config=self.config)
        return I, Q, gains, self.config #, q1_fit_cosine, pi_amp, self.config
...
class AmplitudeRabiProgram(AveragerProgramV2):
    def _initialize(self, cfg):
        ...
        self.add_pulse(ch=qubit_ch, name="pi_f2res",
                       style="arb",
                       envelope="ramp",
                       freq=cfg['qubit_freq_ftores'],
                       phase=cfg['qubit_phase'],
                       gain=cfg['pi_f2res_amp'],
                       )
    ...
    def _body(self, cfg):
        ...
        self.pulse(ch=self.cfg["qubit_ch"], name="pi_f2res", t=0)  # play probe pulse
