...
study = 'QZE_T1_readout_pulse_test'
...
for QubitIndex in Qs_to_look_at:
    for repeat_round in range(3):
        sub_study = f'round{repeat_round}_10us'
       ...
       # Get the config for this qubit
        ...
        pulse_gains=np.linspace(0.1, res_gain[QubitIndex], 100)
        reps = np.linspace(600, 200, 100)
        num=0
        for gain in pulse_gains:
            ############################################## Start IBM like experiment ###########################################
            exp = deepcopy(experiment) #before updating for qze
            exp.readout_cfg['res_gain_ge'][QubitIndex] = gain
            ###################################################### T1 ######################################################
            if run_flags["t1"]:
                ...
                t1 = T1Measurement(QubitIndex, tot_num_of_qubits, studyDocumentationFolder, j, signal, save_figs,
                                   ...
                                   qubit_to_increase_reps_for=qubit_to_increase_reps_for,
                                   multiply_qubit_reps_by=multiply_qubit_reps_by,
                                   verbose=verbose, logger=rr_logger, unmasking_resgain=unmask, adjust_reps_to = int(reps[num]))
